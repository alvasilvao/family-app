  ...
  <div style="display: flex; flex-direction: column; height: 100dvh; background: #f7f3ee">
    ...
    <div class="safe-top" style="background: #2d6a4f; flex-shrink: 0">
      <div style="padding: 10px 20px 0; display: flex; align-items: center; gap: 10px">
        <NuxtLink to="/" style="color: #fff; text-decoration: none; font-size: 20px; display: flex; align-items: center">&larr;</NuxtLink>
        <h1 style="font-family: 'Fraunces', serif; font-size: 18px; font-weight: 600; color: #fff; flex: 1">Weekly Plan</h1>
      </div>
      <div style="padding: 10px 24px 12px; display: flex; align-items: center; justify-content: center">
        <WeekNavigator :week-key="currentWeek" @prev="goWeek(-1)" @next="goWeek(+1)" @go-today="goToday()" />
      </div>
      <!-- Sub-tabs: Plan / Grocery -->
      <div style="display: flex; padding: 0 20px; gap: 4px">
        ...
        >
          ...
          <span
            ...
            style="background: #fff; color: #2d6a4f; border-radius: 999px; font-size: 10px; padding: 1px 6px; font-weight: 700"
          >
            {{ totalServings }}
          </span>
        </button>
        ...
        <div style="padding: 16px 20px 12px">
          <div style="display: flex; justify-content: space-between; margin-bottom: 4px">
            ...
            <div style="display: flex; align-items: center; gap: 10px">
              ...
              <button
                v-if="checkedCount > 0"
                style="font-size: 11px; color: #9b9590; background: none; border: none; cursor: pointer; font-family: 'DM Sans', sans-serif; padding: 8px 4px"
...
const activeTab = computed({
  get: () => (['plan', 'grocery'].includes(route.query.tab as string) ? route.query.tab as string : 'plan'),
  set: (val: string) => {
...
function onSwipe(dir: 'left' | 'right') {
  if (dir === 'left' && activeTab.value === 'plan') activeTab.value = 'grocery'
  if (dir === 'right' && activeTab.value === 'grocery') activeTab.value = 'plan'
}
...
const { basket, groceryChecked, totalServings, fetchPlan, add: planAdd, remove: planRemove, toggleGroceryItem, clearGroceryChecked } = usePlan()
