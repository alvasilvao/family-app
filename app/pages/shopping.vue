<template>
  <div style="display: flex; flex-direction: column; height: 100dvh; background: #f7f3ee">
      ...
      <div v-else>
        ...
        <template v-for="group in favoriteGroups" :key="group.label">
          ...
          >
            ...
            <template v-else>
              <div
                style="display: flex; align-items: center; gap: 11px; flex: 1; min-width: 0; cursor: pointer"
                @click="startEditFavorite(fav)"
              >
    ...
    <!-- Remove favorite confirmation popup -->
    <BaseModal v-if="confirmingFavDelete" style="z-index: 200" @close="confirmingFavDelete = null">
      <div style="padding: 24px; text-align: center">
        <p style="font-size: 15px; color: #1a1a1a; margin: 0 0 6px; font-weight: 600">Remove favorite?</p>
        <p style="font-size: 13.5px; color: #9b9590; margin: 0 0 20px">
          "{{ confirmingFavDelete.name }}" will be removed from your favorites.
        </p>
        <div style="display: flex; gap: 10px; justify-content: center">
          <button
            style="flex: 1; padding: 10px; border-radius: 10px; border: 1px solid #e0d8d0; background: #fff; color: #1a1a1a; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif"
            @click="confirmingFavDelete = null"
          >
            Cancel
          </button>
          <button
            style="flex: 1; padding: 10px; border-radius: 10px; border: none; background: #c0392b; color: #fff; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif"
...
const { favorites, loading: favLoading, fetchFavorites, addFavorite, updateFavorite, removeFavorite } = useFavorites()
...
const confirmingFavDelete = ref<FavoriteIngredient | null>(null)

async function confirmRemoveFavorite() {
  if (!confirmingFavDelete.value) return
  const id = confirmingFavDelete.value.id
  confirmingFavDelete.value = null
  addedFavIds.value.delete(id)
  await removeFavorite(id)
}

function startEditFavorite(fav: FavoriteIngredient) {
  editingFavId.value = fav.id
  editingFavName.value = fav.name
}

function cancelEditFavorite() {
  editingFavId.value = null
  editingFavName.value = ''
}

async function saveEditFavorite() {
  const name = editingFavName.value.trim()
  if (!name || !editingFavId.value) return
  await updateFavorite(editingFavId.value, name)
  editingFavId.value = null
  editingFavName.value = ''
}
